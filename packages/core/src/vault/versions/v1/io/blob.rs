...
pub fn write_blob(rw: &mut Rw, mut reader: impl Read, keyring: &Keyring) -> Result<BlobRef> {
    rw.seek_end()?;
    ...
    loop {
        ...
        let offset = seal_frame(rw, AadDomain::BlobChunk, &buffer, keyring)?;
    ...
    let manifest_offset = seal_frame(rw, AadDomain::BlobManifest, &manifest_bytes, keyring)?;
...
mod tests {
    ...
    fn blob_roundtrip_chunked() {
        ...
        let blob_ref = write_blob(&mut io, &mut payload.as_slice(), &keyring).expect("write blob");
