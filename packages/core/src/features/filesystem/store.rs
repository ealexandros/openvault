...
mod tests {
    ...
    #[test]
    fn new_store_contains_root_folder_metadata() {
        let store = FilesystemStore::new();

        let root = store.folder(&ROOT_FOLDER_ID).expect("root folder metadata");
        assert_eq!(root.name, "/");
    ...
    #[test]
    fn snapshot_contains_all_file_and_folder_metadata() {
        let mut store = FilesystemStore::new();

        let docs_id = Uuid::new_v4();
        store
            .add_folder(FolderMetadata::new(docs_id, Some(ROOT_FOLDER_ID), "docs"))
            .expect("add docs folder");

        let file_id = Uuid::new_v4();
        let mut file = FileMetadata::new(file_id, docs_id, "report.pdf", 4_096);
        file.mime_type = Some("application/pdf".to_string());
        file.content_hash = Some("sha256:abcd".to_string());

        store.add_file(file.clone()).expect("add file metadata");

        let snapshot = store.snapshot();
        assert_eq!(snapshot.folders.len(), 2);
        assert_eq!(snapshot.files.len(), 1);
        assert!(snapshot.folders.contains_key(&ROOT_FOLDER_ID));
        assert_eq!(snapshot.files.get(&file_id), Some(&file));
    }

    #[test]
    fn add_file_rejects_name_conflict_with_folder() {
        let mut store = FilesystemStore::new();
        ...
        let conflicting_file = FileMetadata::new(Uuid::new_v4(), ROOT_FOLDER_ID, "docs", 512);
        let err = store.add_file(conflicting_file).expect_err("name conflict");

        assert!(matches!(err, FilesystemError::NameConflict { .. }));
    }
    ...
    fn delete_folder_rejects_non_empty_folder() {
        ...
        let file_id = Uuid::new_v4();
        store
            .add_file(FileMetadata::new(file_id, docs_id, "todo.txt", 16))
            .expect("add file");

        let err = store.delete_folder(docs_id).expect_err("non-empty folder");
        assert!(matches!(err, FilesystemError::FolderNotEmpty(_)));
    }

    #[test]
    fn restore_replays_deltas_and_clears_sync_state() {
        let docs_id = Uuid::new_v4();
        let file_id = Uuid::new_v4();
        ...
        let snapshot = FilesystemSnapshot::new(folders, HashMap::new());
        let deltas = vec![FilesystemDelta::FileAdded(FileMetadata::new(
            file_id,
            docs_id,
            "readme.md",
            128,
        ...
        let store = FilesystemStore::restore(snapshot, deltas).expect("restore");
        assert!(store.file(&file_id).is_some());
        assert!(store.pending_changes().is_none());
    }
}
